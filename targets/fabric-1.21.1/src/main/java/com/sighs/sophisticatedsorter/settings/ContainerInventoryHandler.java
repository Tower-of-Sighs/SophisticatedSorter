 ...
 * player inventory).
 * <p>
 * <b>Mirror model:</b> the real inventory is canonical for reads, core's tracking stack list is
 ...
 * </ul>
 * Core's internal mutation paths read the tracking stack list directly, so the public read override
...
public class ContainerInventoryHandler extends InventoryHandler {
	...
	private final ContainerInventoryHandle realInventory;
	private final boolean isPlayer;
	/** Whether the save handler (file write) is allowed to run; mirrors core's persistent flag. */
	private boolean persistent = true;

	public ContainerInventoryHandler(int slots, IStorageWrapper wrapper, ContainerSettingsKey key,
			ContainerInventoryHandle realInventory, CompoundTag contentsNbt,
			Runnable saveHandler, StackUpgradeConfig stackUpgradeConfig) {
	...
	public static ContainerInventoryHandler forBlockEntity(int slots, IStorageWrapper wrapper, ContainerSettingsKey key,
			ContainerInventoryHandle realInventory, CompoundTag contentsNbt, Runnable saveHandler,
	...
	public void reload() {
		if (realInventory != null) {
			for (int slot = 0; slot < Math.min(getSlotCount(), realInventory.getSlots()); slot++) {
				// setSlotStack is core's low-level stack writer (the sorter path uses it too), which
				// keeps the tracking copy in sync without going through the public mutation API.
				setSlotStack(slot, realInventory.getStackInSlot(slot).copy());
			}
	...
	public void onContentsChanged(int slot) {
		...
		// real-inventory pointer present: core's deserializeNBT fills the tracking list while
