 ...
 * records the key of the container the player's menu resolved to on
 * {@link PlayerContainerEvent.Open}, and the gate only applies to slots whose container is not the
 * player {@link Inventory} (main, armor, offhand).</li>
...
public final class ContainerMemorySlotGuard {
	...
	public static Boolean rejectIfMemoryMismatch(Slot slot, ItemStack stack) {
		...
		MemorySettingsCategory memory = memoryCategory(player, guard.key);
		if (memory == null) {
			return null;
		}
	...
	@Nullable
	private static MemorySettingsCategory memoryCategory(ServerPlayer player, ContainerSettingsKey key) {
		...
		// handler and render-data suppliers are never dereferenced by a memory read.
		...
		ContainerSettingsHandler handler = new ContainerSettingsHandler(
				ContainerSettingsHandler.fromNbt(storage.getOrCreateContents(key), player.level().registryAccess()),
				() -> {},
				() -> null,
				() -> null);
	...
	/**
	 * Registers the {@link PlayerContainerEvent.Close} listener that clears any stale armed state so
	 * a leftover guard can never gate a later menu. Called from the mod constructor.
	 */
	public static void register() {
		NeoForge.EVENT_BUS.addListener(ContainerMemorySlotGuard::onContainerClose);
	}
