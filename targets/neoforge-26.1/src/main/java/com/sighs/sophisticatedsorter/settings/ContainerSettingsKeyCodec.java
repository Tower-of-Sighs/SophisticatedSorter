 ...
 * utility so the payload classes and the menu registration stay thin. 26.1 port of the shared
 * {@code common/src/minecraft} helper.
 */
public final class ContainerSettingsKeyCodec {
	...
	public static void writeNullable(ContainerSettingsKey key, FriendlyByteBuf buf) {
		if (key == null) {
			buf.writeByte(-1);
		} else {
			key.write(buf);
