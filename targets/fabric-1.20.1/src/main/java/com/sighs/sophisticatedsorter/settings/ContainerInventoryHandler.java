...
/**
 * A write-through {@link InventoryHandler} over an arbitrary container's real item storage (or the
 * player inventory).
 * <p>
 * <b>Mirror model:</b> the real inventory is canonical for reads, core's tracking copy is canonical
 * for writes/tracking. Concretely:
 * <ul>
 ...
 * <li>Every mutation goes through core's own machinery ({@code setStackInSlot}, which keeps the
 * slot tracker, partitioner and nbt snapshot coherent) and is then pushed into the real inventory
 * via {@link #syncToRealInventory}.</li>
 * </ul>
 * Core's internal mutation paths read the tracking copy directly, so the public read override
 * cannot corrupt them; and the settings screen never writes to the real inventory behind this
 * handler's back. Only the sorter (later stage) mutates the real inventory directly - it must call
 * {@link #syncToRealInventory(int)} after each slot change so this handler's tracking copy follows
 * (it also drives the file save through core's own {@code onContentsChanged}).
 * <p>
 ...
 * <p>
 * Platform adaptation: the 1.21.1 reference is written against NeoForge's
 * {@code IItemHandlerModifiable}; on Fabric 1.20.1 the real inventory is the Porting Lib
 * {@link SlottedStackStorage} that this core uses for its item-transfer APIs, and the
 * {@code insertItem}/{@code extractItem} item-handler overrides of the reference do not exist here
 * (core's transfer-API write paths all funnel through {@code setStackInSlot}).
 */
public class ContainerInventoryHandler extends InventoryHandler {
	private final ContainerSettingsKey key;
	private final SlottedStackStorage realInventory;
	private final boolean isPlayer;
	/** Whether the save handler (file write) is allowed to run; mirrors core's persistent flag. */
	private boolean persistent = true;

	public ContainerInventoryHandler(int slots, IStorageWrapper wrapper, ContainerSettingsKey key, SlottedStackStorage realInventory,
	...
	public static ContainerInventoryHandler forBlockEntity(int slots, IStorageWrapper wrapper, ContainerSettingsKey key,
			SlottedStackStorage realInventory, CompoundTag contentsNbt, Runnable saveHandler, StackUpgradeConfig stackUpgradeConfig) {
	...
	@Override
	protected boolean isAllowed(ItemVariant variant) {
		return true;
	}
	...
	/** Whether this handler wraps the real item storage of a block entity (or another container). */
	 ...
	 * core mutation path ({@code setStackInSlot}).
	 */
	...
	/**
	 * Re-reads every slot from the real inventory into this handler's tracking copy. The 1.21.1
	 * reference wrote the private {@code stacks} list directly to bypass the change-tracking/save
	 * machinery; this core keeps that list private, so the reload goes through {@code setStackInSlot}
	 * with persistence temporarily disabled (the write-through half writes the same stack back, which
	 * is idempotent).
	 */
	public void reload() {
		if (realInventory != null) {
			setPersistent(false);
			try {
				for (int slot = 0; slot < Math.min(getSlotCount(), realInventory.getSlotCount()); slot++) {
					setStackInSlot(slot, realInventory.getStackInSlot(slot).copy());
				}
			} finally {
				setPersistent(true);
			}
	...
	public void onContentsChanged(int slot) {
		...
		// real-inventory pointer present: core's deserialization fills the tracking copy while
