...
public class ContainerSettingsHandler extends SettingsHandler {
	...
	protected void addItemDisplayCategory(Supplier<InventoryHandler> inventoryHandlerSupplier, Supplier<RenderInfo> renderInfoSupplier,
		...
		// The 1.21.1 reference passes an extra "allowItemDisplay config" boolean here; the 1.20.1 core
		// ItemDisplaySettingsCategory constructor has no such parameter (the item-display tab is
		// disabled for arbitrary containers anyway, so no behavior is lost).
