...
public final class ContainerSettingsTracker {
	...
	public static void register() {
		MinecraftForge.EVENT_BUS.register(ContainerSettingsTracker.class);
	}
	...
	public static void onPlayerContainerOpen(PlayerContainerEvent.Open event) {
		...
		// On Forge 1.20.1 getContainer() already returns the AbstractContainerMenu directly.
		...
		NetworkHandler.sendToClient(serverPlayer, new ClientboundTrackedContainerKeyPayload(key));
	...
	public static void onPlayerContainerClose(PlayerContainerEvent.Close event) {
		if (event.getEntity() instanceof ServerPlayer serverPlayer) {
			...
			NetworkHandler.sendToClient(serverPlayer, new ClientboundTrackedContainerKeyPayload(null));
		}
	}
