 ...
 * <p>
 * The cache also hosts the one-shot state the settings-screen open flow stages for the client menu
 * constructor: Forge 1.20.1's open-screen packet carries no extra data, so the server pushes the
 * key, the real slot count and the current settings contents ahead of the menu open
 * ({@code com.sighs.sophisticatedsorter.network.ClientboundOpenContainerSettingsPayload}) and
 * {@link com.sighs.sophisticatedsorter.settings.ContainerSettingsContainerMenu#fromBuffer} pops it
 * (see {@link #stageOpenData} / {@link #pollOpenData}).
 */
public final class ClientContainerSettingsCache {
	...
	@Nullable
	private static OpenData pendingOpenData;
	...
	/**
	 * Stages the data the settings screen needs to rebuild its wrapper for the player: the target
	 * key, the real slot count of the container and its current settings contents. Called by the
	 * server-pushed open payload right before the menu open; consumed once by
	 * {@code ContainerSettingsContainerMenu.fromBuffer}.
	 */
	public static void stageOpenData(ContainerSettingsKey key, int slots, @Nullable CompoundTag contents) {
		pendingOpenData = new OpenData(key, slots, contents);
	}

	/** Returns and clears the staged open data (null when none was staged). */
	@Nullable
	public static OpenData pollOpenData() {
		OpenData data = pendingOpenData;
		pendingOpenData = null;
		return data;
	}

	/** The one-shot open state staged by {@link #stageOpenData}. */
	public record OpenData(ContainerSettingsKey key, int slots, @Nullable CompoundTag contents) {
	}
}