...
public final class ContainerInventoryHandles {
	...
	public static final class PlayerInventoryHandle implements IItemHandlerModifiable {
		...
		public ItemStack insertItem(int slot, ItemStack stack, boolean simulate) {
			...
			boolean compatible = existing.isEmpty() || ItemStack.isSameItemSameTags(existing, stack);
			if (!compatible) {
				return stack;
			}
		...
		public int getSlotLimit(int slot) {
			ItemStack existing = getStackInSlot(slot);
			// Container#getMaxStackSize has no item parameter on 1.20.1; the per-item limit is what
			// the player inventory actually enforces for the given stack (default 64 when empty).
			return existing.isEmpty() ? inventory.getMaxStackSize() : existing.getMaxStackSize();
		}
