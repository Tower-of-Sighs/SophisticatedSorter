 ...
 * target's config. A record is "screen class|sort X|sort Y|transfer X|transfer Y".
 */
public final class ButtonPositionCodec {
    ...
    /**
     * Parses one record for the requested screen class. Returns null when the
     * record is malformed or belongs to a different screen class, which makes
    ...
    public static ButtonPositions parse(String record, String screenType) {
        ...
        if (fields.length != 5 || fields[0].isEmpty()
                || (screenType != null && !fields[0].equals(screenType))) {
            return null;
        }
        try {
